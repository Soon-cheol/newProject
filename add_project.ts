...
const mathalive_lib = path.resolve(CONTENT, './mathalive_lib');
...
function _check_folder() {
	...
	checkFdr = path.resolve(mathalive_lib, `./${PREFIX}/${project}`);
	if(fs.existsSync(checkFdr)) throw `'${checkFdr}' 가 존재합니다.`;
...
function _modify_student_import_html() {
	const basePath = path.resolve(mathalive_lib, './P/base_template/student/import.html');
	const tgtPath = path.resolve(mathalive_lib, `./${PREFIX}/${project}/student/import.html`);
...
function _modify_student_init_s_js() {
	const basePath = path.resolve(mathalive_lib, './P/base_template/student/js/init.s.js');
	const tgtPath = path.resolve(mathalive_lib, `./${PREFIX}/${project}/student/js/init.s.js`);
	let src = fs.readFileSync(basePath, 'utf-8');
	src = src.replace(/P\/base_template/g, `${PREFIX}/${project}`);
	return {path: tgtPath, src};
}
function _modify_teacher_import_html() {
	const basePath = path.resolve(mathalive_lib, './P/base_template/teacher/import.html');
	const tgtPath = path.resolve(mathalive_lib, `./${PREFIX}/${project}/teacher/import.html`);
	let src = fs.readFileSync(basePath, 'utf-8');
	src = src.replace(/P\/base_template/g, `${PREFIX}/${project}`);
	return {path: tgtPath, src};
}
function _modify_teacher_init_t_js() {
	const basePath = path.resolve(mathalive_lib, './P/base_template/teacher/js/init.t.js');
	const tgtPath = path.resolve(mathalive_lib, `./${PREFIX}/${project}/teacher/js/init.t.js`);
	let src = fs.readFileSync(basePath, 'utf-8');
	src = src.replace(/P\/base_template/g, `${PREFIX}/${project}`);
	return {path: tgtPath, src};
}
...
async function _copy_template() {
	...
	const srcFdr = path.resolve(mathalive_lib, `./P/base_template`);

	const chkDir = path.resolve(mathalive_lib, `./${PREFIX}`);
	if(!fs.existsSync(chkDir)) {
		fs.mkdirSync(chkDir);
	}

	const tgtFdr = path.resolve(mathalive_lib, `./${PREFIX}/${project}`);
	await _copyFdr(srcFdr, tgtFdr);
}
