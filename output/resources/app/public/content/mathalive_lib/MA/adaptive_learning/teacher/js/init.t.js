(function(app_o, comm_o, tsoc_o){
	...
	function _load(){
		return new Promise(function(resolve, reject){
			app_o.importLink('/content/mathalive_lib/MA/adaptive_learning/teacher/import.html').then(
				function(doc){
	...
	my_o.init = function(){
		_load().then(
			function(){
				...
				app_o.dlvp_info = tsoc_o.dlvp_info;
				
				app_o.al_worker = new Worker('/content/mathalive_lib/MA/adaptive_learning/teacher/js/teacher_worker.js');
				app_o.al_worker.onmessage = function(event) {
					console.log('onmessage', event.data.wkmessage)
					if(event.data.wkmessage) {
						app_o.receive({
							type: 11,
							data: { msgtype: event.data.wkmessage[0], res: event.data.wkmessage[1], params: event.data.wkmessage[2], url: event.data.wkmessage[3] }
						});
					}
				};
				app_o.render();
