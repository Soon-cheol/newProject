(function(app_o, psoc_o){
	...
	function _load(){
		return new Promise(function(resolve, reject){
			app_o.importLink('/content/mathalive_lib/MA/adaptive_learning/student/import.html').then(
				function(doc){
	...
	my_o.init = function(){
		_load().then(			
			function(){
				...
				app_o.al_worker = new Worker('/content/mathalive_lib/MA/adaptive_learning/student/js/student_worker.js');
				app_o.al_worker.onmessage = function(event) {
					if(event.data.wkmessage) {
						console.log('wkmessage :: ', event.data);
						app_o.receive({
							type: 10,
