...
interface IKTextArea {
	...
	onChange?: (text: string, idx?: number|undefined) => void;
...
export class KTextArea extends React.Component<IKTextArea> {
	...
	private _onChange = () => {
        ...
        this._checkLimit();
        this._calc();
        if(this.props.onChange) this.props.onChange(this.m_textarea.value, this.props.idx);
	}
	private _onDone = () => {
		if(!this.m_textarea) return;
	...
	public componentDidUpdate(prev: IKTextArea) {
		if(this.props.on && !prev.on) {
			...
			if(this.m_textarea) {
				if(this.props.prevText) {
					this.m_textarea.value = this.props.prevText;
				} else {
