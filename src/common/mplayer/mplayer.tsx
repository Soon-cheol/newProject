...
class MPlayer {
	...
	@computed get volume() {return this.m_volume; }
	...
	public mediaInited(el: IMedia) {
			...
			if (el.duration > 0) {
				...
				if(this.m_maxTime <= 0) {
					...
					if(this.m_bPlay_temp) {
						...
						this.m_media.currentTime = 0;/*2020_11_02 Remote 버전 관련 추가 */
						this._setState(MPRState.READY);
						this._playOrPause();
		...
		el.onvolumechange = (e: Event) => {
			this.m_volume = el.volume;
		};
